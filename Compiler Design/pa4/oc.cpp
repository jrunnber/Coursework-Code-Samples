...
*/
   const char* execname = basename (argv[0]);
     ...
     FILE* tok_file = open_toks(dupName);
     yyparse();
     fclose(tok_file);
     cpp_pclose();
     //printf(yylval);
   ...
   chomp(filename, 't');
   chomp(filename, 's');
   char *symInName = strcat(filename, "sym");
   const char *symName = symInName;
   symOutfile = fopen(symName, "w");
   postOrder_typecheck(parse_root);
   chomp(filename, 'm');
   chomp(filename, 'y');
   chomp(filename, 's');
   char *astInName = strcat(filename, "ast");
   FILE* astFile;
   const char *astName = astInName;
   astFile = fopen(astName, "w");
   astree::print(astFile, parse_root, 0);
   return EXIT_SUCCESS;
}
