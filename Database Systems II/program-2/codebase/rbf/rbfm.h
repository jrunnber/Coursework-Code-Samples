...
#define ATTR_NULL         10
#define ATTR_NOT_IN_DESC  11
#define OP_NOT_FOUND      12
#define UPDATE_RECORD_DNE 13
...
public: 
  RID lastRID;
  SlotDirectoryHeader lastSlotHead;
  void * pageData;
  ...
  static RBFM_ScanIterator *_rbfm_it;

  //static RBFM_ScanIterator* instance(FileHandle &fileHandle, vector<Attribute> &recordDescriptor, string &conditionAttribute, CompOp compOp, void *value, vector<string> &attributeNames);

  RBFM_ScanIterator(FileHandle &gfileHandle, const vector<Attribute> &grecordDescriptor, const string &gconditionAttribute, const CompOp gcompOp, const void *gvalue, const vector<string> &gattributeNames);

  ~RBFM_ScanIterator();

  bool fieldIssNull(char *nullIndicator, int i);
...
******************************************************************************************************************************************************************/
  ...
  RC scan(FileHandle &fileHandleCons,
      const vector<Attribute> &recordDescriptorCons,
      const string &conditionAttributeCons,
      const CompOp compOpCons,                  // comparision type such as "<" and "="
      const void *valueCons,                    // used in the comparison
      const vector<string> &attributeNamesCons, // a list of projected attributes
...
private:
  ...
  void removeRecord(void *page, SlotDirectoryRecordEntry recordEntry);
};
