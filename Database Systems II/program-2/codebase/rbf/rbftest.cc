...
int RBFTest_8(RecordBasedFileManager *rbfm) 
{
    ...
    //Custom test start
    prepareRecord(recordDescriptor.size(), nullsIndicator, 7, "Record2", 222, 221.22, 2200, record, &recordSize);
    cout << endl << "Inserting Data:" << endl;
    rbfm->printRecord(recordDescriptor, record);
    ...
    prepareRecord(recordDescriptor.size(), nullsIndicator, 48, "dootdootdootdootdootdootdootdootdootdootdootdoot", 1717, 177.817, 6200, record, &recordSize);
    rbfm->updateRecord(fileHandle, recordDescriptor, record, save);

     rc = rbfm->readRecord(fileHandle, recordDescriptor, save, returnedData);
    assert(rc == success && "Reading a record should not fail.");

    printf("Pagenum of updated record is still: %d\n", save.pageNum);
...
int RBFTest_9(RecordBasedFileManager *rbfm, vector<RID> &rids, vector<int> &sizes) {
    ...
    printf("Starting Custom Test\n");
    vector<string> tempString;
    for(int i = 0; i < recordDescriptor.size(); i++){
        tempString.push_back(recordDescriptor[i].name);
    }

    //edit

    RBFM_ScanIterator* scanI = (RBFM_ScanIterator*) malloc(sizeof(struct RBFM_ScanIterator));
    ...
    zeroID.pageNum = 0;
    printf("start delete\n");
    rbfm->deleteRecord(fileHandle, recordDescriptor, zeroID);
    printf("end Delete");

    rbfm->scan(fileHandle, recordDescriptor, "Int4", LT_OP, &doot, tempString, *scanI);

    int j = 0;
    while(scanI->getNextRecord(tempRID, tempData) != RBFM_EOF){
        if(0 == 0){
